  ...
  async function handleTestConnection() {
    ...
    try {
      if (window.clarity) {
        ...
        setTestResult({ ok: true, message: `Connected — ${tasks.length} issues found` });
        onSyncComplete();
      }
    } catch (err) {
      setTestResult({
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[440px] max-w-[90vw] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/10 bg-slate-900/95 p-6 shadow-2xl backdrop-blur-xl">
          ...
          <Dialog.Description className="mt-1 text-xs text-slate-400">
            Connect your Atlassian account to sync Jira issues as tasks.
          </Dialog.Description>

          <div className="mt-5 space-y-4">
            <Field label="Jira Domain" placeholder="your-team" suffix=".atlassian.net" value={domain} onChange={setDomain} />
            <Field label="Email" placeholder="you@company.com" value={email} onChange={setEmail} type="email" />
            <Field label="API Token" placeholder="Paste your Atlassian API token" value={token} onChange={setToken} type="password" />
          ...
          <div className="mt-6 flex items-center justify-end gap-2">
            <button
              ...
              className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-xs font-medium text-slate-200 transition hover:bg-white/10 disabled:opacity-40 disabled:pointer-events-none"
            >
              {testing ? (
            ...
            <button
              ...
              className="rounded-lg bg-indigo-500/20 px-4 py-2 text-xs font-medium text-indigo-200 transition hover:bg-indigo-500/30 disabled:opacity-40 disabled:pointer-events-none"
            >
              {saving ? (
                <span className="flex items-center gap-1.5">
